...
public class Main {
	...
	private static void solve() {
		...
		tree = new int[H * 2][];
		for (int i = 0; i < N; i++)
			tree[i + H] = new int[] { io.nextInt() };
		for (int i = H + N; i < tree.length; i++)
			tree[i] = new int[] { Integer.MAX_VALUE };
		build(1, H, 1);
	...
	private static int query(int L, int R, int K) {
		int l = 0;
		int r = tree[1].length - 1;

		while (l < r) {
			int mid = (l + r) / 2;

			if (count(1, H, 1, L, R, tree[1][mid]) < K)
				l = mid + 1;
			else
				r = mid;
		}
	...
	private static void build(int l, int r, int i) {
		...
		tree[i] = new int[left.length + right.length];
		int p = 0;
		int pl = 0;
		int pr = 0;
	...
	private static int upperBound(int[] array, int K) {
		int l = 0;
