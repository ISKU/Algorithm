...
public class Main {
	...
	private static ArrayList<Integer>[] tree, pbs, singers;
	private static long[] bit;
	private static int[] singer, start, end, l, r;
	private static Data[] datas;
	private static int N, K, order;
	...
	private static void solve() {
		...
		for (int i = 1; i <= N; i++)
			singers[i] = new ArrayList<Integer>();
		for (int i = 1; i <= N; i++) {
			singer[i] = io.nextInt();
			singers[singer[i]].add(i);
		}
		...
		while (finish) {
			...
			for (int i = 0; i < K; i++) {
				int root = datas[i].P;
				long weight = datas[i].S / (end[root] - start[root] + 1);
				update(start[root], weight);
				update(end[root] + 1, -weight);

				for (int j = 0; j < pbs[i].size(); j++) {
					finish = true;
					int singer = pbs[i].get(j);

					long sum = 0;
					long aim = J * singers[singer].size();
					for (int idx : singers[singer]) {
						sum += sum(start[idx]);
						if (sum >= aim)
							break;
					}
	...
	private static class Data {
		...
		public Data(int T, int P, long S) {
			this.T = T;
			this.P = P;
			this.S = S;
		}
