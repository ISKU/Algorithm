...
public class Main {
	...
	private static long[] tree, lazy1, lazy2;
	...
	private static void solve() {
		...
		while (M-- > 0) {
			...
			int e = io.nextInt();
			if (cmd == 4) {
				io.println(sum(1, H, 1, s, e));
				continue;
			}

			long v = io.nextLong();
			update(1, H, 1, s, e, cmd, v);
		}
	}

	private static void update(int l, int r, int i, int L, int R, int cmd, long value) {
		propagate(l, r, i);
		...
		if (L <= l && r <= R) {
			if (cmd == 1)
				lazy2[i] = (lazy2[i] + value) % MOD;
			if (cmd == 2) {
				lazy1[i] = (lazy1[i] * value) % MOD;
				lazy2[i] = (lazy2[i] * value) % MOD;
			}
			if (cmd == 3) {
				lazy1[i] = 0;
		...
		int mid = (l + r) / 2;
		update(l, mid, i * 2, L, R, cmd, value);
		update(mid + 1, r, i * 2 + 1, L, R, cmd, value);
		tree[i] = (tree[i * 2] + tree[i * 2 + 1]) % MOD;
	}
	...
	private static long sum(int l, int r, int i, int L, int R) {
		...
		int mid = (l + r) / 2;
		return (sum(l, mid, i * 2, L, R) + sum(mid + 1, r, i * 2 + 1, L, R)) % MOD;
	}

	private static void propagate(int l, int r, int i) {
		if (lazy1[i] == 1 && lazy2[i] == 0)
			return;

		if (i < H) {
			lazy1[i * 2] = (lazy1[i * 2] * lazy1[i]) % MOD;
			lazy1[i * 2 + 1] = (lazy1[i * 2 + 1] * lazy1[i]) % MOD;
			lazy2[i * 2] = (lazy1[i] * lazy2[i * 2] + lazy2[i]) % MOD;
			lazy2[i * 2 + 1] = (lazy1[i] * lazy2[i * 2 + 1] + lazy2[i]) % MOD;
		}

		tree[i] = (tree[i] * lazy1[i] + (r - l + 1) * lazy2[i]) % MOD;
		lazy1[i] = 1;
		lazy2[i] = 0;
	}
