...
public class Main {
	...
	public static void solve() {
		...
		while (M-- > 0) {
			int u = io.nextInt();
			int v = io.nextInt();
			long w = io.nextLong() * 2;
			graph[u].add(new Edge(v, w));
			graph[v].add(new Edge(u, w));
		}

		long[] fox = dijkstraForFox();
		long[][] wolf = dijkstraForWolf();
		int count = 0;
		for (int i = 2; i <= N; i++)
	...
	private static long[] dijkstraForFox() {
	...
	private static long[][] dijkstraForWolf() {
		...
		while (!pq.isEmpty()) {
			Edge u = pq.poll();
			if (dist[u.state][u.vertex] < u.weight)
				continue;

			int state = 1 - u.state;
			for (Edge v : graph[u.vertex]) {
				long cost = u.weight + ((u.state == 0) ? v.weight / 2 : v.weight * 2);
				if (cost < dist[state][v.vertex]) {
					dist[state][v.vertex] = cost;
					pq.add(new Edge(v.vertex, cost, state));
				}
			}
		}
	...
	private static class Edge implements Comparable<Edge> {
		...
		public Edge(int vertex, long weight, int state) {
			this.vertex = vertex;
			this.weight = weight;
			this.state = state;
		}
