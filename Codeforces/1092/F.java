...
public class F {
	...
	private static long[] values, up, down;
	...
	private static void solve() {
		...
		long answer = 0;
		for (int i = 1; i <= N; i++)
			answer = Math.max(answer, up[i]);
		io.print(answer);
	}

	private static void dfs(int u, int parent, int h) {
		down[u] = values[u];
		up[1] += values[u] * h;
	...
	private static void dfs2(int u, int parent) {
		if (u != 1)
			up[u] = up[parent] - down[u] + (down[1] - down[u]);
