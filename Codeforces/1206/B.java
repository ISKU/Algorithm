...
public class B {
	...
	private static void solve() {
		int N = io.nextInt();
		int[] array = io.nextIntArray(N);

		int zeroCount = 0;
		int minusCount = 0;
		long sum = 0;
		for (int i = 0; i < N; i++) {
			long minus = Math.abs(array[i] + 1);
			long plus = Math.abs(1 - array[i]);
			sum += Math.min(minus, plus);
		...
		io.println((minusCount % 2 == 0) ? sum : ((zeroCount >= 1) ? sum : sum + 2));
	}

	private static class Node {
		public int index;
		public long minus, plus;
		public long distance;

		public Node(int index, long minus, long plus, long distance) {
			this.index = index;
			this.minus = minus;
			this.plus = plus;
